...
if (typeof exports === "object") {
  ...
  Showdown.parse = function (md, gh, preloadUrls, preloadTitles ) {
    var converter = new Showdown.converter();
    return converter.makeHtml(md, gh, preloadUrls, preloadTitles );
  };
}
...
this.makeHtml = function(text, gh , preloadUrls, preloadTitles ) {
...
//
    ...
    // GS01: 
    // preload the urls and titles with passed in values
    if( typeof(preloadUrls) == "object" ) {
        for(var ndx in preloadUrls) {
            g_urls[ndx] = preloadUrls[ndx];
        }
    }

    if( typeof(preloadTitles) == "object" ) {
        for(var ndx in preloadTitles) {
            g_titles[ndx] = preloadTitles[ndx];
        }
    }
...
var _HashHTMLBlocks = function(text) {
    ...
    // GS01: Add in escape for triple backtick (why is that not in here already? 
    text = text.replace(/^(```\s*.*\s*$[\s|\S]*?^```)$/gm, hashElement);
    //text = text.replace(/\n(```\s*.*\s*$[\s|\S]*?\n```)$/g, "XXXXX");
