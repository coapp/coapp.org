...
(function ($) {
    ...
    var Item = Class.extend({
        ...
        init: function (entry) {
            var id = entry.find('id').eq(0).text();
            id = id.replace(/(:|\.|\||;|\[|\])/g,'-');
            this.id = id;
    ...
    var Link = Class.extend({
        ...
        init: function (entry) {
            if (entry != null) {
                ...
                this.rel = entry.attr('rel');
                this.type = entry.attr('type');
            }
        }
    });
    ...
    var Package = Class.extend({
        ...
        init: function (entry) {
            if (entry != null) {
                ...
                this.canonicalName = entry.attr('CanonicalName');
                this.name = entry.attr('Name');
                this.packageType = entry.attr('PackageType');
                this.flavor = entry.attr('Flavor');
                ...
                var bindingPolicy = entry.find('[nodeName="coapp:BindingPolicy"]').eq(0);

                this.bindingPolicyMinVersion = bindingPolicy.attr('Minimum');
                this.bindingPolicyMaxVersion = bindingPolicy.attr('Maximum');

                //this.bindingPolicyMaxVersion = entry.find('[nodeName="coapp:BindingPolicyMaxVersion"]').eq(0).text();
                
                var dependencies = new Array();

                entry.find('[nodeName="coapp:Dependencies"]').eq(0).find('item').each(function () {
                    var dep_id = jQuery(this).attr('key');
                    dep_id = dep_id.replace(/(:|\.|\||;|\[|\])/g,'-');
                    dependencies.push( dep_id );

                    /*jQuery(this).find('[nodeName="coapp:List"]').eq(0).find('[nodeName="coapp:Uri"]').each(function () {
                        var dep_list = jQuery(this).text();
                    });*/
                });
                ...
                entry.find('[nodeName="coapp:Feeds"]').eq(0).find('[nodeName="coapp:Uri"]').each(function () {
                    feeds.push( jQuery(this).text() );
                });
    ...
    var Role = Class.extend({
        ...
        init: function (entry) {
            if (entry != null) {
                this.name = entry.attr('Name');
                this.packageRole = entry.attr('PackageRole');
            }
        }
    });
